...
import { FetchJobApplicantsRequest, Job } from './types'
...
const jobApiSlice = jobApi.injectEndpoints({
  endpoints: build => ({
    ...
    fetchJobApplicantsById: build.query<
      IListApiResponse<Job>,
      FetchJobApplicantsRequest
    >({
      query: ({ jobId, query }) => {
        const params = getPaginationQueryParams(query)
        return {
